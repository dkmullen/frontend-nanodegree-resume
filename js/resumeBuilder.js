...
var bio = {
    ...
    "skills" : [
		"HTML/CSS", "JavaScript & JQuery", "MS Office", "Teaching", 
        "Adminisration"
		],
...
bio.display = function() { //makes a function that is part of the bio object
	...
	$("#header").append(bioPic, formattedWelcomeMsg);
    //adds contact info to top element...
...
work.display = function() {
    if (work.jobs.length > 0) {   
    	for (var job in work.jobs) {
    		...
    		var formattedEmployerLink = formattedEmployer.replace("#", 
                work.jobs[job].url);
...
projects.display = function() {
    if (projects.projects.length > 0) {
    	for (var project in projects.projects) {
    		...
    		var formattedTitleURL = formattedTitle.replace("#", 
    		...
    		//the following adds images to each project, but only if there 
            //are images; allows for varying numbers of images too.
...
education.display = function() {
    if (education.schools.length > 0) {
    	for (var school in education.schools) {
    		...
    		var formattedSchoolLink = formattedSchoolName.replace("#", 
    		...
    		$(".education-entry:last").append(formattedSchoolLink + formattedSchoolDegree, 
    	...
    	for (var course in education.onlineCourse) {
    		...
    		var formattedOnlineLink = formattedOnlineTitle.replace("#", 
                education.onlineCourse[course].url);
    		//adds the url to the previous
    		var formattedOnlineSchool = HTMLonlineSchool.replace("%data%", 
                education.onlineCourse[course].school);
    		...
    		var formattedOnlineURL = HTMLonlineURL.replace("%data%", 
                education.onlineCourse[course].url);
    		var formattedOnlineURLLink = formattedOnlineURL.replace("#", 
                education.onlineCourse[course].url);
    		//adds the url to the previous
    		
    		$(".education-entry:last").append(formattedOnlineLink + formattedOnlineSchool, 
                formattedOnlineDates, formattedOnlineURLLink);
    	}
    }
};
education.display();
